...
@section('content')
    <div class="mx-auto max-w-3xl">
        <a href="{{ route('admin.devices.show', $device) }}" class="mb-8 inline-flex items-center gap-2 text-sm font-medium text-slate-500 transition-colors hover:text-slate-900">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            ...
            @endif
            <form method="POST" action="{{ route('admin.devices.update-details', $device) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" @selected(old('owner_id', $device->current_owner_id) == $user->id)>
                ...
                <div class="grid gap-6 sm:grid-cols-2">
                    <div>
                        ...
                        @error('imei')
                            <p class="mt-2 text-xs text-rose-600">{{ $message }}</p>
                        @enderror
                        <p class="mt-2 text-xs text-slate-500">IMEI must be exactly 15 digits.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="brand" class="mt-2 h-12 w-full rounded-lg border border-slate-200 bg-white px-4 text-sm">
                            ...
                            @foreach ($brands as $brand)
                                <option value="{{ $brand }}" @selected(old('brand', $device->brand) === $brand)>{{ $brand }}</option>
                            @endforeach
                        </select>
                        @error('brand')
                            <p class="mt-2 text-xs text-rose-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="device_type" class="mt-2 h-12 w-full rounded-lg border border-slate-200 bg-white px-4 text-sm">
                        ...
                        @foreach ($deviceTypes as $type)
                            <option value="{{ $type['value'] }}" @selected(old('device_type', $device->device_type) === $type['value'])>{{ $type['label'] }}</option>
                        @endforeach
                    </select>
                    @error('device_type')
                        <p class="mt-2 text-xs text-rose-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <div class="mt-3 grid gap-4 sm:grid-cols-2">
                        <label class="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 bg-slate-50/60 p-4">
                            <input type="radio" name="purchase_type" value="new" class="h-4 w-4" @checked(old('purchase_type', $device->purchase_type) === 'new') />
                            <div>
                        ...
                        <label class="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 bg-slate-50/60 p-4">
                            <input type="radio" name="purchase_type" value="secondhand" class="h-4 w-4" @checked(old('purchase_type', $device->purchase_type) === 'secondhand') />
                            <div>
                ...
                <div class="grid gap-6 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="date" name="purchase_date" value="{{ old('purchase_date', $device->purchase_date?->format('Y-m-d')) }}" class="mt-2 h-12 w-full rounded-lg border border-slate-200 bg-white px-4 text-sm" />
                        @error('purchase_date')
                            <p class="mt-2 text-xs text-rose-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        @error('invoice')
                            <p class="mt-2 text-xs text-rose-600">{{ $message }}</p>
                        @enderror
                        <p class="mt-2 text-xs text-slate-500">Current: {{ $device->invoice_path ? 'Uploaded' : 'Not provided' }}</p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input
                        name="seller_name"
                        value="{{ old('seller_name', $device->seller_name) }}"
