    ...
    <div class="mx-auto max-w-6xl space-y-6">
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <div class="border-b border-slate-200 bg-slate-50/60 px-6 py-4">
                <form method="GET" action="{{ route('admin.imei-logs.index') }}" class="grid gap-3 md:grid-cols-12">
                    <div class="md:col-span-9">
                        <label class="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Search IMEI</label>
                        <input
                            type="text"
                            name="q"
                            value="{{ request('q') }}"
                    ...
                    <div class="flex items-end gap-2 md:col-span-3">
                        <button class="h-10 w-full rounded-lg bg-[color:var(--brand-600)] px-3 text-xs font-semibold text-white">Search</button>
                        <a class="h-10 w-full rounded-lg border border-slate-200 px-3 text-xs font-semibold text-slate-600 flex items-center justify-center" href="{{ route('admin.imei-logs.index') }}">Reset</a>
                    </div>
                </form>
            </div>
